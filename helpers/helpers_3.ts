...
import { ERROR } from '@/store/actions/transport/transportActionTypes';
import {
  DOWNLOAD_VIDEOS_SUCCESS,
  VIDEO_LOADING,
} from '@/store/actions/video/videoActionTypes';
...
export const downloadByUrl = (
  ...
  fetch(url)
    ...
    .then((blob) => {
      ...
      dispatch({ type: DOWNLOAD_VIDEOS_SUCCESS });
    })
    .catch((err) => {
      dispatch({ type: ERROR });
      console.log(err);
